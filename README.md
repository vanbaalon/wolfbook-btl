...
| `src/native/addon.cpp` | N-API wrapper. Exports two JS functions: `boxToLatex(str, opts?)` and `lineBreakLatex(latex, opts?)`, plus two string properties: `version` (semver from `package.json`) and `buildDate` (UTC `YYYY-MM-DD`, embedded at compile time via `build_version.h`). Zero DOM dependencies. |
...
│       ├── addon.cpp            N-API entry point (boxToLatex, lineBreakLatex, version, buildDate)
│       ├── build_version.h      generated by build.sh — version + build date (not committed)
